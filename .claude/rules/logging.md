...
Properly use DEBUG and ERROR levels to record logs that are useful only for understanding what happened or errors that need to be analyzed.
...
**Example:**
```typescript
// ✅ Prefer - logs go to stdout/stderr
...
Never log sensitive data such as names, addresses, and credit card numbers.
...
Always be clear in log messages, without overdoing it or using long texts.
...
try {
  ...
  // only logs but doesn't re-throw when it should
  console.error(error);
}
...
// This facilitates queries in log systems:
// - Filter by specific userId
// - Search orders above a value
// - Group by source
```
